"""
Need help with the main loop:
- I want to modify the logic to log the next fetch hour just once, while checking for schedule pending
tasks more often
"""
...
def log_next_update():
    next_time = datetime.now() + timedelta(hours=Config.FECTH_INTERVAL_HOURS)
...
def main():
    ...
    schedule.every(Config.FECTH_INTERVAL_HOURS).hours.do(fecth_and_store)
    schedule.every(Config.FECTH_INTERVAL_HOURS).hours.do(log_next_update)
